...
var equipmentMapConfig = [
	...
	{configId:"body_0",image:"body_png"}
...
class HeroPanel extends egret.DisplayObjectContainer{
	...
	public addTouchEvent() {
		...
		this.equipmentMap.body.touchEnabled = true;
		this.equipmentMap.body.addEventListener(egret.TouchEvent.TOUCH_BEGIN,this.onTapBody,this);
	...
	onTapBody(e:egret.TouchEvent) {
		console.log("body");	
		var body:Equipment;
		for(var i=0; i<this.hero.equipments.length; i++) {
			if(this.hero.equipments[i].property.type == equipmentType.BODY) {
				body = this.hero.equipments[i];
			}
		}
